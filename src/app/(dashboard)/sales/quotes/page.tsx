...
export default function SalesQuotesPage() {
  ...
  const refreshRows = React.useCallback(async () => {
    const items = await fetchSalesDocumentsApi("quote");
    setAllRows(items);
  }, []);

  React.useEffect(() => {
    void refreshRows().catch((error) => {
      toast.error(error instanceof Error ? error.message : "Failed to load quotes.");
    });
  }, [refreshRows]);
    ...
    <PageShell>
      ...
      <div className="p-6 space-y-4">
            ...
            selectedIds.length > 0 ? (
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  onClick={async () => {
                    try {
                      await bulkDocumentActionApi("quote", "approve", selectedIds);
                      await refreshRows();
                      toast.success("Quote(s) approved.");
                      setSelectedIds([]);
                    } catch (error) {
                      toast.error(error instanceof Error ? error.message : "Failed to approve quotes.");
                    }
                  }}
                >
                  Approve
                ...
                <Button
                  ...
                  onClick={() =>
                    downloadCsv(
                      `quotes-selected-${new Date().toISOString().slice(0, 10)}.csv`,
                      filtered
        ...
        <DataTable<SalesDocRow>
          data={filtered}
          columns={columns}
          onRowClick={(row) => router.push(`/docs/quote/${row.id}`)}
