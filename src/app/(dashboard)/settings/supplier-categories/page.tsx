...
import {
  fetchSupplierCategoriesApi,
  createSupplierCategoryApi,
  updateSupplierCategoryApi,
  type SupplierCategoryRow,
} from "@/lib/api/supplier-categories";
import { toast } from "sonner";
import * as Icons from "lucide-react";

export default function SupplierCategoriesPage() {
  const [categories, setCategories] = React.useState<SupplierCategoryRow[]>([]);
  const [loading, setLoading] = React.useState(true);
  const [sheetOpen, setSheetOpen] = React.useState(false);
  const [editing, setEditing] = React.useState<SupplierCategoryRow | null>(null);

  const refresh = React.useCallback(async () => {
    setLoading(true);
    try {
      const items = await fetchSupplierCategoriesApi();
      setCategories(items);
    } catch (e) {
  ...
  const openEdit = (row: SupplierCategoryRow) => { setEditing(row); setSheetOpen(true); };

  const handleSave = async (values: { code: string; name: string; description: string; isActive: boolean }) => {
    try {
      if (editing) {
        await updateSupplierCategoryApi(editing.id, {
          name: values.name,
  ...
  return (
    <PageShell>
      <PageHeader
        ...
        description="Group vendors the same way Tally groups sundry creditors — packaging, raw material, logistics, and the rest."
        breadcrumbs={[
          ...
          <CardContent className="p-0">
            {loading ? (
              ...
              <div className="p-8 text-center text-sm text-muted-foreground">
                No supplier categories yet. Add one above to assign to suppliers.
              </div>
            ) : (
              <Table>
                <TableHeader>
...
function CategorySheet({
  ...
  initial: SupplierCategoryRow | null;
    ...
    <Sheet open onOpenChange={(o) => !o && onClose()}>
      <SheetContent>
        <SheetHeader>
          ...
          <SheetDescription>
            Supplier categories group creditors for purchasing and AP reporting.
          </SheetDescription>
        </SheetHeader>
